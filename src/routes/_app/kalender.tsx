...
function KalenderPage() {
  ...
  const [form, setForm] = useState({ judul: "", deskripsi: "", lokasi: "", wilayah: "", mulai: "", selesai: "", kategori: "", urgensi: "sedang" });
  ...
  const add = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("kegiatan").insert({ ...form, created_by: user!.id } as never);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Kegiatan ditambahkan"); setShow(false); qc.invalidateQueries({ queryKey: ["kegiatan"] }); },
  ...
  return (
    <div>
      ...
      {show && (
        <Panel title="Tambah Kegiatan" className="mb-4">
          ...
          <button onClick={() => add.mutate()} disabled={add.isPending || !form.judul || !form.mulai}
            className="mt-3 px-4 py-2 bg-primary text-primary-foreground text-xs font-mono-display rounded disabled:opacity-50">SIMPAN</button>
        </Panel>
      )}
        ...
        {items?.map(k => (
          <div key={k.id} className="panel scanline p-4">
            <div className="flex items-start justify-between gap-2 mb-2">
              ...
              <Badge variant={URGENSI_VARIANT[k.urgensi as keyof typeof URGENSI_VARIANT ?? "sedang"]}>{k.urgensi ?? "sedang"}</Badge>
            </div>
